...
module id_stage(
    ...
    assign id_alutype_o[0] = inst_add|inst_subu|inst_slt|inst_mfhi|inst_mflo|inst_addiu|inst_sltiu|inst_lb|inst_lw|inst_sb|inst_sw;

    // 内部操作码aluop
    assign id_aluop_o[7]   = inst_lb | inst_lw | inst_sb | inst_sw;
    assign id_aluop_o[6]   = 1'b0;
    assign id_aluop_o[5]   = inst_slt | inst_sltiu;
    assign id_aluop_o[4]   = inst_add | inst_subu | inst_and | inst_mult | inst_sll |inst_ori | inst_addiu | inst_lb | inst_lw | inst_sb | inst_sw;
    assign id_aluop_o[3]   = inst_add | inst_subu | inst_and | inst_mfhi | inst_mflo |inst_ori |inst_addiu | inst_sb | inst_sw;
    assign id_aluop_o[2]   = inst_slt | inst_and | inst_mult | inst_mfhi | inst_mflo |inst_ori | inst_lui | inst_sltiu;
    assign id_aluop_o[1]   = inst_subu |inst_slt|inst_sltiu|inst_lw|inst_sw;
    assign id_aluop_o[0]   = inst_subu | inst_mflo | inst_sll |inst_ori | inst_lui | inst_addiu | inst_sltiu;

    // 写通用寄存器使能信号
    assign id_wreg_o       = inst_and|inst_add|inst_subu|inst_slt|inst_mfhi|inst_mflo|inst_sll|inst_ori|inst_lui|inst_addiu|inst_sltiu|inst_lb|inst_lw;
    ...
    wire sext              = inst_addiu | inst_sltiu |inst_sb|inst_sw ; 
    wire upper             = inst_lui;
    // 读通用寄存器堆端口1使能信号
